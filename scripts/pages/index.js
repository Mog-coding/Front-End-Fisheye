...
/*RECUP DES DONNEES */
fetch('photo.json')   // 1ere promise résolue qd serveur distant repond
    .then(function (response) {   // 2eme promise résolue qd data chargée
        return response.json();
    })
    .then(function (resp) {//2eme promis donne data format json vert objet, then
        const { photographers } = resp; // bien mettre meme nom objet {data: [{}
        const dataFac = photographers.map(function (el) {
            return new Photographer(el)
        }
        )
        dataFac.forEach(function (el) {
            const Template = new PhotoCard(el);
            console.log("contenu du noeud CLASS");
            console.log(Template.createPhotoCard());
            document.querySelector(".photographer_section").appendChild(Template.createPhotoCard());
        })
    }
    );
